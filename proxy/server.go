...
type BinapiResponse struct {
	Msg  api.ReplyMessage
	Msgs []api.ReplyMessage
}
...
func (s *BinapiRPC) Invoke(req BinapiRequest, resp *BinapiResponse) error {
	...
	if req.IsMulti {
		...
		for {
			...
			msg := reflect.New(reflect.TypeOf(req.ReplyMsg).Elem()).Interface().(api.ReplyMessage)
		...
		// create new message in response of type ReplyMsg
		resp.Msg = reflect.New(reflect.TypeOf(req.ReplyMsg).Elem()).Interface().(api.ReplyMessage)
