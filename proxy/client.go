...
func (b *BinapiClient) Invoke(_ context.Context, request api.Message, reply api.ResponseMessage) error {
...
func (r *requestCtx) ReceiveReply(msg api.ResponseMessage) error {
	return invokeInternal(r.rpc, r.req, msg, r.timeout)
}

func invokeInternal(rpc *rpc.Client, msgIn api.Message, msgOut api.ResponseMessage, timeout time.Duration) error {
	req := BinapiRequest{
		Msg:      msgIn,
		ReplyMsg: msgOut,
		Timeout:  timeout,
...
type multiRequestCtx struct {
	...
	index   int
	replies []api.ResponseMessage
}

func (r *multiRequestCtx) ReceiveReply(msg api.ResponseMessage) (stop bool, err error) {
