...
func GenerateDoc(gen *Generator) {
	logf("----------------------------")
	logf(" Generate doc.go")
	logf("----------------------------")
	filename := "doc.go"
	g := gen.NewGenFile(filename, GoImportPath(gen.opts.ImportPrefix))
	...
	g.P()
	pkgname := "binapi"
	g.P("// Package " + pkgname + " is a containing package for the binapi generated apis")
	g.P("package ", pkgname)
}
